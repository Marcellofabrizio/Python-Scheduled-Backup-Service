...
def dump_db(host_name, port, user, database, password):
    ...
    dump_command = 'pg_dump -h {0} -p {1} -U {2} -w -d {3} > dump_files/{4}'\
...
def create_time_stamp(database=None):
    ...
    file_name = (database + '_' + str(time_stamp) + '.sql')
    return file_name.replace('-', '_').replace(' ', '_').replace(':', '_').replace('_db_', '_')
